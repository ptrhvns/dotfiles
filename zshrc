...
ZSH_SPECTRUM_TEXT=${ZSH_SPECTRUM_TEXT:-This is a color test.}

# Show all 256 colors with color number.
function print_foreground_colors() {
  for code in {000..255}; do
    print -P -- "$code: %{$CLRFG[$code]%}$ZSH_SPECTRUM_TEXT%{$reset_color%}"
  done
}
...
function print_background_colors() {
  for code in {000..255}; do
    print -P -- "$code: %{$CLRBG[$code]%}$ZSH_SPECTRUM_TEXT%{$reset_color%}"
  done
}
